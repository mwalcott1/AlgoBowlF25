...
# NOTE: RIGHT NOW, THIS ONLY ITERATES ONCE, AS GRAVITY IS NOT YET IMPLEMENTED.
while True:
    ...
    # first filter down (note that the top of our grid is stored at grid[0] and the bottom at grid[r-1])
    for i in range(r):
        for j in range(c):
            # if this square is missing, move down everything above it
            if (i, j) in currentBlock:
                for k in range(i):
